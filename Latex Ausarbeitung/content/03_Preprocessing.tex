...
The remaining features were analysed regarding their pearson correlation. Only two pairs of features with substantive amount of data (less than 25\% NaNs) have a very strong correlation (\>80\%).  These are cp_4\<-\>painexer and rldv5\<-\>rldv5e. The high correlation between cp_4, which was oneHotEncoded from the categorical variable cp, describes wether the patient has no chest pain at rest. Painexer describes wether the patient only has pain when exercising. Therefore the correlation seems intuitive. We kept both variables because 
